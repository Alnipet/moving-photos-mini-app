...
import {Avatar, Group, NavIdProps, Panel, PanelHeader, PanelHeaderBack, SimpleCell, usePlatform, View} from '@vkontakte/vkui'
...
export const Home: FC<HomeProps> = ({fetchedUser, userData, albums, photos, handleGetPhoto}) => {
  ...
  const handleMovePhoto = async (arg: { albumId: number, selectedPhotoIds: string[] }) => {
    ...
    console.log(arg)
    const fetchedMove = Promise.all(arg.selectedPhotoIds.map(id => {
  ...
  return (
    <>
      {platform === 'vkcom' && fetchedUser && (
        <Group>
          <SimpleCell before={photo_200 && <Avatar src={photo_200} />} subtitle={city?.title} expandable="auto">
      ...
      <View activePanel={activePanel}>
        <Panel mode={'plain'} id={'albumList'}>
          <AlbumsList albums={albums} handleGetPhoto={async (id) => {
        ...
        <Panel id={`photoList`}>
          <PanelHeader
            before={<PanelHeaderBack label={platform === 'vkcom' ? 'Назад' : undefined} onClick={() => {
