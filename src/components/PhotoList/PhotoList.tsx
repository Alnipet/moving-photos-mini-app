...
import {Avatar, Button, CardGrid, ContentCard, CustomSelectOption, Group, Header, Select, Text} from '@vkontakte/vkui'
    ...
    <>
      <Group mode="card">
        ...
        <div className={styles.moveBtnWrapper}>
          ...
          <Button style={{marginLeft: 15}} size={'m'} disabled={!selectedAlbumId || !selectedPhotoId.length} onClick={() => {
            if (!selectedAlbumId) return
            handleMovePhoto({albumId: selectedAlbumId, selectedPhotoIds: selectedPhotoId}).then(() => {
              setSelectedPhotoId([])
            })
          }
          }>Перенести фото</Button>
        </div>
